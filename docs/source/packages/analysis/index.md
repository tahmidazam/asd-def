...
| `attribute` | The refit-era attribution: decomposes each stratum's re-estimated movement onto features and categories, and contrasts the probands that moved with those that stayed. Archived; the category attribution ($H_0^F$) is now read from the single cached fit by the block engine. | {doc}`The refit pilot <archive/tracking-the-classes-across-strata>` |
...
| `invariance-trajectory` | Recasts the invariance read as a null-free effect size: the separation-scaled local-centroid displacement along an axis, with a family-clustered bootstrap band, an in-plane capture fraction, a control-panel specificity check, the per-class directional net-trend statistic with a descriptive break, the per-category decomposition (the block engine's $H_0^F$ read), and (era only) the current-versus-retrospective referent split. | {doc}`$H_0^A$: invariance <hypotheses/h0a-invariance>`, {doc}`$H_0^F$: category attribution <hypotheses/h0f-attribution-categories>`, {doc}`Computing the invariance effect size <guides/computing-the-invariance-effect-size>` |
...
- {doc}`$H_0^F$: is the drift spread evenly across the seven categories? <hypotheses/h0f-attribution-categories>` Rejected: concentrated, and concentrated differently by class (developmental for the developmental class, social-communication and internalizing for the rest).
